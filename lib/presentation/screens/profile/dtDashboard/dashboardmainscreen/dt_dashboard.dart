...
// import 'dt_dashboard_helper.dart';
// import 'dt_dashboard_edit_details.dart';
...
//                         initialLabelIndex: seasons[seasonIndex].isPublish,
//                         inactiveBgColor:
...
//                   seasonIndex,
//                 ),
//                 children: seasons[seasonIndex].episodes?.isEmpty ?? true
//                     ? []
//                     : [
//                         for (var i = 0;
//                             i < seasons[seasonIndex].episodes!.length;
//                             i++)
//                           ListTile(
//                               onTap: () {},
//                               leading: Text(
...
//                               ),
//                               title: Text(
//                                 seasons[seasonIndex].episodes?[i].name ?? "NA",
//                                 style: const TextStyle(color: Colors.white),
//                               ),
//                               trailing: DashBoardHelper().popupEpi(
//                                   context: context,
//                                   episode: seasons[seasonIndex].episodes![i],
...
//                     final provider = Provider.of<DTDashboardProvider>(context,
//                         listen: false);
//                     try {
//                       await provider.deleteSeason(
//                         digiID: seasons?[index].dtId.toString() ?? '0',
//                         seasonID: seasons?[index].id.toString() ?? '0',
//                       );
//                     } finally {
...
import 'package:filmox_clean_architecture/domain/entity/digitalTheater/dashboard/dt_dashboard_entity.dart';
import 'package:filmox_clean_architecture/presentation/providers/digitalTheater/dt_dashboard_provider/dt_dashboard_provider.dart';
import 'package:filmox_clean_architecture/presentation/screens/profile/dtDashboard/castscreen/EditCast.dart';
import 'package:filmox_clean_architecture/widgets/custom_popups.dart';
import 'package:filmox_clean_architecture/widgets/loading_screen.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
...
import 'package:provider/provider.dart';
import 'package:toggle_switch/toggle_switch.dart';
import '../crewscreen/EditCrew.dart';
import '../dtDashboardMFUpload/AddSeasons.dart';
import '../dtDashboardSFUpload/EditMovieInfo.dart';
import '../dtDashboardSFUpload/EditTrailer.dart';
import '../dtDashboardBasicDetails/dt_dashboard_edit_details.dart';
import 'dt_dashboard_helper.dart';

class DtDashboardScreen extends StatefulWidget {
  final String digitalTheaterID;

  const DtDashboardScreen({super.key, required this.digitalTheaterID});

  @override
  State<DtDashboardScreen> createState() => _DtDashboardScreenState();
}

class _DtDashboardScreenState extends State<DtDashboardScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback(
      (timeStamp) => Provider.of<DTDashboardProvider>(context, listen: false)
        ..fetchDashboardDetails(digitalTheaterID: widget.digitalTheaterID),
    );
  }

  @override
  Widget build(BuildContext context) {
    final digitalTheaterProvider =
        Provider.of<DTDashboardProvider>(context, listen: true);
    final dtProvider = digitalTheaterProvider.digitalTheaterDashBoardEntity;
    return Scaffold(
                ...
                ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    errorWidget: (context, url, error) =>
                        Image.asset(AppConstants.filmoxLogo),
                    progressIndicatorBuilder:
                        (context, url, downloadProgress) =>
                            CircularProgressIndicator(
                                value: downloadProgress.progress),
                    imageUrl: dtProvider!.poster,
                  ),
                ),
                SizedBox(width: 10.w),
                Expanded(
  ...
  Widget multiplefileUploadItems(DtDashboardEntity? dtProvider) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        ...
        // ),
        ListView.builder(
          itemCount: dtProvider.seasons?.length,
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          padding: EdgeInsets.all(10.dg),
          itemBuilder: (
              ...
              children: [
                Dismissible(
                  ...
                  onDismissed: (direction) async {
                    ...
                    try {
                      await provider.deleteSeason(
                        digiID: dtProvider.id.toString(),
                        seasonID:
                            dtProvider.seasons?[index].id.toString() ?? "",
                      );
                    } finally {
                      customSuccessToast(context, "Changes saved !!");
                    }
                  ...
                  child: Container(
                    margin: EdgeInsets.symmetric(vertical: 10.h),
                    child: seasonItem(index, dtProvider.seasons ?? []),
                  ),
                ),
                SizedBox(
                  height: 10.h,
